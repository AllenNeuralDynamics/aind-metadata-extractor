...
class FiberData(BaseModel):
    ...
    experimenter_full_name: Optional[List[str]] = Field(default_factory=list, description="List of experimenter names")
    ...
    rig_id: Optional[str] = Field(None, description="Identifier for the experimental rig")
    mouse_platform_name: Optional[str] = Field(None, description="Name of the mouse platform used")
    active_mouse_platform: Optional[bool] = Field(
        None, description="Whether the mouse platform was active during the session"
    )
    data_streams: Optional[List[dict]] = Field(default_factory=list, description="List of data stream configurations")
    session_type: Optional[str] = Field(default="FIB", description="Type of session")
    iacuc_protocol: Optional[str] = Field(None, description="IACUC protocol identifier")
    ...
    protocol_id: Optional[List[str]] = Field(default_factory=list, description="List of protocol identifiers")
    data_directory: Optional[Union[str, Path]] = Field(
        None, description="Path to data directory containing fiber photometry files"
    )
    data_files: Optional[List[str]] = Field(default_factory=list, description="List of data file paths")
    rig_config: Optional[dict] = Field(default=None, description="Rig configuration dictionary")
    session_config: Optional[dict] = Field(default=None, description="Session configuration dictionary")
