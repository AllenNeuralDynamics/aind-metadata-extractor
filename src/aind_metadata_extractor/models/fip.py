...
class FiberData(BaseModel):
    ...
    experimenter_full_name: Optional[List[str]] = Field(
        default_factory=list, description="List of experimenter names"
    )
    session_start_time: Optional[datetime] = Field(
        None, description="Start time of the session"
    )
    session_end_time: Optional[datetime] = Field(
        None, description="End time of the session"
    )
    subject_id: Optional[str] = Field(
        None, description="Subject identifier"
    )
    rig_id: Optional[str] = Field(
        None, description="Identifier for the experimental rig"
    )
    mouse_platform_name: Optional[str] = Field(
        None, description="Name of the mouse platform used"
    )
    ...
    data_streams: Optional[List[dict]] = Field(
        default_factory=list, description="List of data stream configurations"
    )
    session_type: Optional[str] = Field(
        default="FIB", description="Type of session"
    )
    iacuc_protocol: Optional[str] = Field(
        None, description="IACUC protocol identifier"
    )
    notes: Optional[str] = Field(
        None, description="Session notes"
    )
    anaesthesia: Optional[str] = Field(
        None, description="Anaesthesia used"
    )
    animal_weight_post: Optional[float] = Field(
        None, description="Animal weight after session"
    )
    animal_weight_prior: Optional[float] = Field(
        None, description="Animal weight before session"
    )
    protocol_id: Optional[List[str]] = Field(
        default_factory=list, description="List of protocol identifiers"
    )
    ...
    rig_config: Optional[dict] = Field(
        default=None, description="Rig configuration dictionary"
    )
    session_config: Optional[dict] = Field(
        default=None, description="Session configuration dictionary"
    )
    ...
    output_filename: str = Field(
        default="session_fip.json", description="Name of output file"
    )