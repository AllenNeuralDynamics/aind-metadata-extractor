...
class TestSmartspimExtractor(unittest.TestCase):
    ...
    def test_extractor_initialization(self):
        ...
        self.assertEqual(extractor.job_settings.metadata_service_path, "https://api.test.com/smartspim")
    ...
    def test_extract_metadata_from_microscope_files(
        self, mock_exists, mock_get_session_end, mock_read_json, mock_isdir, mock_listdir
    ):
        ...
        self.assertEqual(result["session_config"], example_metadata_info["session_config"])
        self.assertEqual(result["wavelength_config"], example_metadata_info["wavelength_config"])
    ...
    @patch("aind_metadata_extractor.smartspim.extractor.os.path.isdir")
    @patch("pathlib.Path.exists")
    def test_extract_metadata_missing_asi_file(self, mock_exists, mock_isdir, mock_listdir, mock_read_json):
    ...
    @patch("aind_metadata_extractor.smartspim.extractor.os.path.isdir")
    @patch("pathlib.Path.exists")
    def test_extract_metadata_missing_metadata_file(self, mock_exists, mock_isdir, mock_listdir, mock_read_json):
    ...
    @patch("aind_metadata_extractor.smartspim.extractor.read_json_as_dict")
    @patch("pathlib.Path.exists")
    def test_extract_metadata_empty_json(self, mock_exists, mock_read_json, mock_isdir, mock_listdir):
    ...
    def test_extract_metadata_invalid_input_source(self):
        ...
        self.assertIn("input_source must be a valid path", str(context.exception))
    ...
    def test_extract_metadata_from_slims_success(self, mock_get):
        ...
        mock_response.json.return_value = {"data": [example_imaging_info_from_slims]}
        ...
        self.assertEqual(call_args[0][0], "https://api.test.com/smartspim")
        self.assertEqual(call_args[1]["params"]["subject_id"], "804714")
    ...
    def test_extract_metadata_from_slims_multiple_results(self, mock_get):
        ...
        mock_response.json.return_value = {
            "data": [example_imaging_info_from_slims, example_imaging_info_from_slims]  # Duplicate
        }
        mock_response.raise_for_status.return_value = None
        mock_get.return_value = mock_response
    ...
    @patch("aind_metadata_extractor.smartspim.extractor.get_session_end")
    @patch("pathlib.Path.exists")
    def test_extract_metadata_invalid_date_format(
        self, mock_exists, mock_get_session_end, mock_read_json, mock_isdir, mock_listdir
    ):
